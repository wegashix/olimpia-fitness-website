...
export function Gallery() {
  return <section className="section gallery-section" id="atmosphere"><div className="container"><SectionLabel number="04">ATMOSPHERE</SectionLabel><div className="gallery-heading"><Reveal><h2>ВНУТРИ<br /><span>OLIMPIA.</span></h2></Reveal><Reveal delay={0.12}><p>Пространство, в котором тренировка становится привычкой.</p></Reveal></div><div className="masonry-gallery">{photos.map(([src, number, label], i) => <Reveal key={src} delay={i * 0.05} className={`gallery-item gallery-item-${i + 1}`}><Image src={src} alt={`Olimpia Fitness Club — ${label.toLowerCase()}`} fill sizes="(max-width: 800px) 100vw, 33vw" /><div className="gallery-shade" /><div className="gallery-meta"><span className="gallery-badge">{number}</span><span className="gallery-badge">{label}</span></div></Reveal>)}</div></div></section>;
}
